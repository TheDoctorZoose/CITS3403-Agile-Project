...
class User(UserMixin, db.Model):
    ...
    messages_sent = db.relationship(
        "Message", foreign_keys="Message.sender_id", back_populates="sender", lazy=True
    )
...
class GameEntry(db.Model):
    ...
    allowed_users = db.relationship(
        "User", secondary=gameentry_permissions, backref="permitted_game_entries"
    )

    likes = db.relationship(
        "Like", backref="entry", lazy="dynamic", cascade="all, delete-orphan"
    )
    favorites = db.relationship(
        "Favorite", backref="entry", lazy="dynamic", cascade="all, delete-orphan"
    )
    comments = db.relationship(
        "Comment", backref="entry", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class PlayerGameEntry(db.Model):
    ...
    name = db.Column(
        db.String(30)
    )  # not stored in user table because it could vary across games (e.g. nicknames)
